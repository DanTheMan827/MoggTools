    ...
    public class MoggCrypt
    {
        ...
        internal static void doCrypt(byte[] key, byte[] moggData, byte[] file_nonce, int oggOffset)
        {
            ...
            using (AesManaged aesManaged = new AesManaged())
            {
                ...
                cryptoTransform.TransformBlock(file_nonce, 0, file_nonce.Length, numArray, 0);
                for (int i = oggOffset; i < moggData.Length; i++)
                {
                    if (num == 16)
                    {
                        num1 = 0;
        ...
        internal static byte[] gen_key(byte[] HvKey, Stream mogg, int version)
        {
            ...
            using (AesManaged aesManaged = new AesManaged())
            {
                ...
                aesManaged.CreateDecryptor().TransformBlock(numArray, 0, numArray.Length, numArray, 0);
            }
            mogg.Position = 20 + num * 8 + 16;
            int num1 = mogg.ReadInt32LE();
            mogg.Position = 20 + num * 8 + 16 + 8;
            int num2 = mogg.ReadInt32LE();
            mogg.Position = 20 + num * 8 + 16 + 48;
            if (version == 17)
            {
        ...
        public static Stream GetOggCryptStream(Stream mogg)
        {
            ...
            int num1 = mogg.ReadInt32LE();
            return new OffsetStream(mogg, num1, mogg.Length - num1);
        }
        ...
        private static byte[] grind_array(int a1, int a2, byte[] bar, int version)
        {
            ...
            for (int j = 0; j < 16; j++)
            {
                ...
                {
                    num3 = MoggCrypt.O_funcs(num3, bar[k + 1], numArray1[numArray3[bar[k]]]);
                }
                bar[j] = num3;
            }
            return bar;
        ...
        private class VorbisCryptStream : Stream
        {
            ...
            public VorbisCryptStream(Stream mogg)
            {
                this.mogg = mogg;
                this.readHeader();
                this.Length = mogg.Length - headerLength;
                this.Seek(0, SeekOrigin.Begin);
            }

            private void decryptBytes(byte[] target, int offset, int count)
            {
                this.mogg.Position = this.position + headerLength;
                if ((this.position - 1) / 16 != this.counterIncrements)
                {
                    this.fixCounter();
                }
                byte[] numArray = this.mogg.ReadBytes(count);
                byte[] numArray1 = new byte[16];
                int num = (int)(this.position % 16);
                using (AesManaged aesManaged = new AesManaged())
                {
                    aesManaged.Mode = CipherMode.ECB;
                    aesManaged.BlockSize = 128;
                    ...
                    ICryptoTransform cryptoTransform = aesManaged.CreateEncryptor();
                    cryptoTransform.TransformBlock(this.counter, 0, counter.Length, numArray1, 0);
                    for (int i = 0; i < count; i++)
                    {
                        if (this.position != 0 && this.position % 16 == 0)
                        {
                            int num1 = 0;
                            for (int j = 0; j < 16; j++)
                            {
            ...
            public override int Read(byte[] buffer, int offset, int count)
            {
                ...
                if (this.position < 32)
                {
                    while (this.position < 32 && count > 0 && offset < buffer.Length)
                    {
                    ...
                    this.mogg.Position = 32 + this.headerLength;
                    this.initialCounter.CopyTo(this.counter, 0);
            ...
            private void readHeader()
            {
                ...
                this.mogg.Position = 16;
                this.numEntries = this.mogg.ReadInt32LE();
                this.mogg.Position = 20 + this.numEntries * 8;
                ...
                {
                    this.mogg.Position = 20 + this.numEntries * 8 + 16;
                    int num = this.mogg.ReadInt32LE();
                    this.mogg.Position = 20 + this.numEntries * 8 + 16 + 8;
