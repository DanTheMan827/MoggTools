...
namespace MoggTools
{
    internal class Song
    {
        ...
        private string make_midi_track(MidiTrack t)
        {
            ...
            Guid guid = Guid.NewGuid();
            return rppTrack.Replace("$GUID$", guid.ToString()).Replace("$NAME$", t.Name).Replace("$VOL$", single.ToString(cultureInfo)).Replace("$PAN$", single1.ToString(cultureInfo)).Replace("$ITEM$", this.make_midi_item(t, midi.TicksPerQN));
        }
